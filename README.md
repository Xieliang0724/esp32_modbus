...
> 急停为**常闭 NC**：DI4/DI5 实时反映触点状态（闭合=1，按下=0）；**0x3000** 为锁存状态（按下→0 锁存，急停松开 + 按复位按钮才恢复 1）。回路断开（断线/未接）一律视为急停触发（fail-safe）。
...
**锁存语义**（仅输入寄存器 0x3000）：急停按下 → 0x3000 变 **0** 并锁存（松开不回 1）；必须满足 **急停均已松开** 且 **按一下复位按钮**（上升沿，一次同时解除两路），0x3000 才回到 1。DI4/DI5 是实时状态、不锁存。急停只上报状态，不联动 DO。复位按钮禁用（-1）时，急停锁存只能断电重启解除。
...
**急停/复位验证流程**：
1. 正常状态：读 DI4/DI5 = `1`（触点闭合）、0x3000 = `1`（正常）
2. 按下急停 → DI4（或 DI5）= `0`（**实时**变化）；0x3000 = `0`（锁存），LED 红灯闪烁
3. 松开急停 → DI4/DI5 回到 `1`（实时）；0x3000 仍为 `0`（已锁存）
4. 按一下复位按钮（DI6 读到 1）→ 0x3000 回到 `1`（解除锁存），LED 恢复绿色
